 ...
 */
@property (nonatomic) CLLocationAccuracy sysLocateAccuracy;

/**
 *	@brief 系统的定位超时时间
 *	@note 默认为10s，假如在超时时间内，系统还未找到符合sysLocateAccuracy精度的要求，将在该超时时间后返回一个精度最高的值(假如有的话)
 */
@property (nonatomic) NSTimeInterval sysLocateTimeOut;
 ...
 */
@property (nonatomic) uint64_t currentUIN;
/** 
 *	@brief 当前的系统获取的GPS位置信息
 *	@note 调用startLocate后更新，初始值nil
 */
@property (nonatomic, readonly) CLLocation* currLocation;
/** 
 ...
 */
- (BOOL)setLocatePurposeString:(NSString *)str;
 ...
 *			- 通知的 @ref QLBSServiceMsg .serviceName = @ref QLBSSERVICE_GETPOSITION
...
/**
 *  @deprecated 请用 @code - (void)cancelRequestGetPosition:(int)reqid @endcode
 *	@brief 取消请求获取当前的位置信息
 *	@param[in] reqid 请求序列号
 *	@return 取消请求是否成功
 */
- (void)cancelRequestGetCurrentPosition:(int)reqid DEPRECATED_ATTRIBUTE;

/**
 *	@brief 请求附近的POI热点信息
 *	@note 这个方法需要在 @code - (BOOL)startLocate @endcode 成功刷新位置信息后调用，假如从未刷新位置，直接放回FALSE
 *			- 通知的 @ref QLBSServiceMsg .serviceName = @ref QLBSSERVICE_GETPOILIST
 ...
 *			- 通知的 @ref QLBSServiceMsg .serviceName = @ref QLBSSERVICE_SEARCHPOILIST
 *			- 通知的 @ref QLBSServiceMsg .result = YES 表示成功 .data = @ref QLBSPoiInfoBatch
 *			- 通知的 @ref QLBSServiceMsg .result = NO 表示失败 根据.errocode和.desc获取失败信息描述
...
#if QLBS_ENABLE_ARBITRARY_GPS_POSITION
/**
 *	@brief 请求获取某个地理位置信息
 *	@note 输出火星坐标与地址
 *			- 通知的 @ref QLBSServiceMsg .serviceName = @ref QLBSSERVICE_GETPOSITION
 *			- 通知的 @ref QLBSServiceMsg .result = YES 表示成功 .data = @ref QLBSPosition
 *			- 通知的 @ref QLBSServiceMsg .result = NO 表示失败 根据.errocode和.desc获取失败信息描述
 *	@param[in] location 请求的地理位置
 *	@param[in] reqid 请求序列号
 *	@return 请求是否成功
 */
- (BOOL)requestGetPosition:(CLLocation *)location requestID:(int)reqid;

/**
 *	@brief 请求获取某个地理的POI热点信息
 *	@note 这个方法需要在 @code - (BOOL)startLocate @endcode 成功刷新位置信息后调用，假如从未刷新位置，直接放回FALSE
 *			- 通知的 @ref QLBSServiceMsg .serviceName = @ref QLBSSERVICE_GETPOILIST
 *			- 通知的 @ref QLBSServiceMsg .result = YES 表示成功 .data = @ref QLBSPoiInfoBatch
 *			- 通知的 @ref QLBSServiceMsg .result = NO 表示失败 根据.errocode和.desc获取失败信息描述
 ...
 *	@return 请求是否成功
 */
- (BOOL)requestGetPoiList:(CLLocation *)location poiTypes:(NSArray*)types radius:(NSUInteger)radius beginPos:(NSUInteger)beginPos reqNum:(NSUInteger)reqNum sortMode:(QLBSPoiSortMode)sortMode requestID:(int)reqid;
#endif
/**
 *	@brief 取消请求获取位置信息
 *	@param[in] reqid 请求序列号
 *	@return 取消请求是否成功
 */
- (void)cancelRequestGetPosition:(int)reqid;

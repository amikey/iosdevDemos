 ...
 *			服务类型包括但不限于以下几种: @ref QLBSSERVICE_LOCATE | @ref QLBSSERVICE_GETPOSITION
 ...
 *			v1.5.2 \n
 *			添加接口 @code - (NSData*)exportPlatformDeviceData:(double)lat longitude:(double)log locateTimeInterval:(double)timeInterval @endcode
 *			v1.5.3 \n
 *			修改过滤缓存GPS的时间间隔为30s \n
 *          v1.5.4 \n
 *          添加接口 @code - (BOOL)requestGetPosition:(CLLocation *)location requestID:(int)reqid @endcode
 *          添加接口 @code - (void)cancelRequestGetPosition:(int)reqid @endcode
 *          修改常量 QLBSSERVICE_GETCURRPOSITION 为 @ref QLBSSERVICE_GETPOSITION \n
 *          修改常量 QLBSSERVICE_GETNEARPOILIST 为 @ref QLBSSERVICE_GETPOILIST \n
 *          修改常量 QLBSSERVICE_SEARCHNEARPOILIST 为 @ref QLBSSERVICE_SEARCHPOILIST \n
 *			v1.5.5 \n
 *			修改定位精度一律为 kCLLocationAccuracyBest \n
 *			添加接口 @code - (BOOL)setLocatePurposeString:(NSString *)str @endcode
 *          v1.5.6 \n
 *          去掉输出定位平台加密数据接口，需要这个接口的，可以直接用QLBSSDKLite.framework，减少库文件大小:)
 */
